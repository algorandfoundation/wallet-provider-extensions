...
- **`WithMigrations`**: Versioned migrations for the persisted provider stores (from [`@algorandfoundation/provider-migrations`](../../migrations)).
- **`WithLogs`**: Unified logging for all operations.
...
- **`WithAccountsKeystore`**: The reference bridge that links accounts to keys in the keystore, enabling signing capabilities (it addresses every key type by its base64 public key).
- **`WithAlgorandAccounts`**: The Algorand-specific layer (from [`@algorandfoundation/algorand-accounts-extension`](../../accounts/algorand-extension)): concrete Algorand addresses, canonical post-quantum digests for Falcon-1024 keys, and live balances/assets via an algod watchlist.
...
- **`WithCredentials`**: Verifiable Credentials held by the wallet (imported from the [`@algorandfoundation/credentials`](../../credentials/meta) meta package, which Metro resolves to [`@algorandfoundation/react-native-credentials`](../../credentials/react-native)); the credential store engine is persisted through a two-line MMKV driver and holder-bound to the identities extension, plus the W3C **Digital Credentials API** seams: the requester side at `provider.credential.digital` (still an explicit `unsupported` stub on React Native) and the wallet/holder registry at `provider.credential.digitalProvider`, backed on Android by the Digital Credentials expo module bundled inside that same package.
- **`WithPasskeys`**: The passkeys held by this device's credential provider (from [`@algorandfoundation/react-native-passkeys`](../../passkeys/react-native)); the native feeder syncs the provider's MMKV-backed credentials (`react-native-passkey-autofill`) into the passkeys store, using the same records and same types, with no key material in JS.
- **`WithPasskeysKeystore`**: The bridge (from [`@algorandfoundation/passkeys-keystore-extension`](../../passkeys/keystore-extension)) that mirrors keystore-derived P256 domain keys into the same passkeys store and propagates passkey removals back to the keystore.
...
- The **Platform Status** section surfaces both seams: `provider.credential.digital.isSupported()` (the _requester_ side, which is still an explicit `unsupported` stub on React Native) and `provider.credential.digitalProvider.isSupported()` (the _wallet registry_, real on Android with Google Play services).
- With the registry supported, `lib/digital-credentials.ts` mirrors every stored SD-JWT VC into the OS credential registry and answers the OpenID4VP presentation requests the platform routes back, meaning any browser, including the [`use-wallet-client`](../use-wallet-client) verifier example, can request a credential from this wallet.
...
The kind comes from `metadata.keyType`, which the `accounts-keystore-extension` bridge records on every account it populates; the wallet also **transmits to connected dapps** (see `exposeConnectionAccounts` in `lib/connections.ts`: the accounts remote mirror's `expose` projection carries each account's `type` + `metadata` verbatim).
...
1. **Install dependencies** (this example lives in a pnpm workspace)
...
- `lib/digital-credentials.ts`: The Digital Credentials holder wiring, consisting of OS registry sync plus the OpenID4VP presentation handler.
- `stores/credentials.ts`: The reactive credentials store plus the two-line MMKV persistence driver.
- `lib/connections.ts`: The Liquid Auth protocol wiring, including native `SignalService` transport, keystore-backed signing/auth seams, STUN/TURN config, and approval dialogs.
- `lib/liquidAuthFlow.ts`: The Liquid Auth WebAuthn ceremony (passkey assertion/attestation + `liquid` extension) run through the native cookie-jar client before peering; it is skipped entirely when the server's `/auth/session` already authenticates the wallet.
- `lib/connectionResume.ts`: The presence-driven auto-resume orchestrator, which silently re-offers persisted sessions when the server reports the dapp waiting.
...
- `lib/passkeyProvider.ts`: Shares the keystore master key + deterministic-P256 main key with the credential provider service; this is required before it can serve or register any passkey.
...
`liquidAuth()` protocol; both are imported from the
...
- WebRTC negotiates over Nodely's public STUN + TURN
  ...
  across networks, and the dapp example passes the same list.
...
- **Reconnection is seamless after the first QR pairing**
  ...
  presence shows the dapp waiting the wallet silently re-offers; specifically, the
...
- If the server session expired, the auto-resume stays quiet (it never
  ...
  path, as that one may re-run the passkey ceremony.
...
- The **Passkeys** screen lists the provider's stored passkeys (shared native
  ...
  this wallet is the device's _active_ provider, and includes a shortcut into the
...
1. the keystore **master key** (`setMasterKey`), enabling the provider service to
   unseal records in the shared `keystore` MMKV instance, and
2. the **passkey main key** id (`setMainKeyId`), which is the deterministic-P256
...
Android Credential Manager, which means it runs through **this wallet's credential
