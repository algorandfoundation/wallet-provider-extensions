...
/**
 * Mints a fresh standalone ed25519 signing key, generated by the host
 * WebCrypto (`react-native-quick-crypto`) and not derived from any seed. The
 * accounts-keystore bridge auto-populates a keystore account for it (keyed
 * by its base64 public key), so minting a key IS minting an account.
 *
 * @param key - The keystore API (`provider.key.store`).
 * @returns The new ed25519 key id.
 *
 * @example
 * ```typescript
 * const keyId = await mintEd25519Key(key.store);
 * ```
 */
export async function mintEd25519Key(key: KeyStore): Promise<KeyId> {
  return key.generate({
    type: "ed25519",
    algorithm: "EdDSA",
    extractable: false,
...
/**
 * Creates a new local identity by deriving the next identity-context key
 * from the wallet's XHD root; the identities extension auto-populates a
 * `did:key` identity from it.
 *
 * Identity keys are XHD (BIP32-Ed25519) children, so they need the XHD
 * root, not the deterministic-P256 "main key" that shares the
 * `hd-root-key` type. HD children are produced through `deriveFromSeed`
 * (which runs the BIP32-Ed25519 shim over the unlocked root) rather than
 * `generate`, which only mints fresh (non-derived) keys. Context 1
 * (Identities) maps to coin type 0 in the BIP44 path; `derivation: 9` is
 * Peikert mode.
 *
 * @param key - The keystore API (`provider.key.store`).
 * @param keys - The current key list (to find the root and the next free index).
 * @returns The new derived identity key id.
 * @throws When no XHD root key exists yet (create a seed first).
 */
export async function createLocalIdentity(key: KeyStore, keys: Key[]): Promise<KeyId> {
  const root = keys.find((k) => k.type === "hd-root-key" && k.metadata?.scheme !== "pbkdf2-p256");
  if (!root) {
    throw new Error("No root key available — create a seed first on the Keys page.");
  }
  const nextIndex = keys.filter(
    (k) => k.metadata?.context === 1 && k.metadata?.parentKeyId === root.id,
  ).length;
  return key.deriveFromSeed!(root.id, `m/44'/0'/0'/0/${nextIndex}`, {
    algorithm: "EdDSA",
    mode: "peikert",
    metadata: {
