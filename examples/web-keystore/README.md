...
extension** for the Wallet Provider, installed via the `@algorandfoundation/keystore`
meta package (whose `browser` condition resolves to `@algorandfoundation/keystore-web`).
It persists key metadata in IndexedDB and runs its cryptography through the host
WebCrypto `SubtleCrypto` plus composable shims.
...
- **Per-key operations**: sign & verify, export public data, and remove, all of which are surfaced
