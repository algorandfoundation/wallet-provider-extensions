...
/**
 * Node.js credentials extension options.
 *
 * The same {@link CredentialStoreOptions} shape: node adds no platform seam of
 * its own to the shared `options.credentials` namespace, so the block accepts
 * the base `store`/`hooks`/`driver`/`binding`/`storageKey`.
 *
 * @example
 * ```typescript
 * const options: NodeCredentialsOptions = {
 *   credentials: { driver: fileDriver, storageKey: "wallet/credentials" },
 * };
 * ```
 */
export type NodeCredentialsOptions = CredentialStoreOptions;

/**
 * The extension surface contributed by the node credentials package: the
 ...
 * provider.credential.digital.isSupported(); // false on node
 * ```
 */
 ...
 * @param options - {@link NodeCredentialsOptions}; every `options.credentials` member is optional.
 * @returns The {@link NodeCredentialsExtension} surface.
 *
 * @example
 * ```typescript
...
export const WithCredentials: Extension<NodeCredentialsExtension> = (
  ...
  options: NodeCredentialsOptions,
) => {
  const binding =
