...
The engine runs fully **standalone**; `createKeyStore` (and the platform
engines built on it) needs no Provider at all. It also ships first-class
support for the Algorand Wallet Provider: each platform package exports a
`WithKeyStore` extension that mounts the same engine on a provider. Both usage
modes are equal citizens of the API.

## Package Map

| Package                                     | Role                                                                    |
| ------------------------------------------- | ----------------------------------------------------------------------- |
| `@algorandfoundation/keystore-core`         | This package, including types, errors, shims, and constants.            |
| `@algorandfoundation/keystore-node`         | Node / server engine (OS keychain driver, CLI, RPC; re-exports core).   |
...
| `@algorandfoundation/keystore`              | Meta package that resolves to the right platform package via `exports`. |
...
- [**`KeyStoreDriver<Ctx>`** / **`DriverCapabilities`** / **`DriverMaterial`**](./src/types/driver.ts): The storage-backend contract, serving as a "material custodian" that owns encrypted-at-rest persistence, metadata and any platform-specific unlock flow.
...
binding; thus, the common case needs no wiring: `createKeyStore({ driver, store })`.
...
- **Self-encryption** (the default): host Subtle AES-GCM keyed off the key's
  **private** material, unlocked just-in-time through the driver; it is never based off
...
- **Peer encryption** (`options.recipientPublicKey` set): an HPKE (RFC 9180)
  **Auth-mode** seal in the `DHKEM(P-256, HKDF-SHA256)` + `HKDF-SHA256` +
  `AES-128-GCM` suite, run entirely through the injected host Subtle; this is a key
...
It further provides
`deriveSharedSecret` (the Diffie-Hellman/ECDH negotiation path, which is routed through
...
is how the Wallet Provider `WithKeyStore` extension threads its hooks in. The
extension is a thin, first-class mount over the very same engine, and nothing in
the engine requires a Provider to be present.
...
| `withSubtleBIP39`      | `BIP39`              | `BIP39Binding`      | Mnemonic seed source: `generateKey` mints recoverable **entropy**, and `deriveBits` yields a 64-byte seed |
...
| `Falcon-1024` | Post-quantum Falcon-1024 signatures (added by the Falcon shim) |
...
| `hd-root-key`        | `raw`         | XHD root key derived from a `seed` (basis for `hd-derived-ed25519`)       |
...
| `ed25519`            | `EdDSA`       | Standalone Ed25519 key generated from fresh randomness (no seed parent)   |
| `falcon-1024`        | `Falcon-1024` | Post-quantum Falcon-1024 signing key derived from a `seed` parent         |
...
> Generation of `hd-root-key` and `falcon-1024` keys requires a seed, either a `seed` (or legacy `hd-seed`) parent via `params.parentKeyId` or inline `params.seed` bytes; callers convert any mnemonic to a seed before calling `generate`. Classical Ed25519 and post-quantum Falcon-1024 signing keys are first-class peers used through the same `sign` / `verify` calls, and the same seed that backs your HD-derived Ed25519 accounts can back a Falcon-1024 key. Unrecognized algorithms fall through to a `SubtleCrypto` WebCrypto fallback.
