...
export class NativeDataChannel {
     ...
     */
    _scheduleFlush() {
        if (this._flushScheduled || this._pending.length === 0)
            return;
    ...
    setState(state) {
        ...
        const readyState = ['connecting', 'open', 'closing', 'closed'].includes(next)
            ? next
            : 'closed';
...
export class NativePeerConnection {
    ...
    close() {
        if (this.iceConnectionState === 'closed' && this.connectionState === 'closed')
            return;
