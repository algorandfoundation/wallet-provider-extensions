...
const WithKeystoreExtension: Extension<any> = (provider) => {
    ...
    Object.defineProperty(provider, "secrets", {
        get() { return store.state.secrets; },
        configurable: true,
        enumerable: true
    });
    return {
        keystore: {
            add: async (key: any) => { addSecret(store, key); return key; },
            remove: async (id: string) => { 
                store.setState(s => ({ secrets: s.secrets.filter(k => k.id !== id) }));
            },
            import: async (key: any) => { addSecret(store, key); return key; },
            export: async (id: string) => { return store.state.secrets.find(k => k.id === id); }
        }
    };
};
...
describe("BIP-39 Crypto Extension README Examples", () => {
  ...
  it("should run the 'Integration with Keystore' example", async () => {
    ...
    const MyProvider = Provider.withExtensions([WithKeystoreExtension, WithBip39CryptoExtension]);
    const provider = new MyProvider({
      id: "test-provider",
      name: "Test Provider",
    }, {
  ...
  it("should run the 'Lifecycle Hooks' example", async () => {
    ...
    cryptoBip39Hooks.before("generate", (options) => {
      logs.push(`Generating a new mnemonic with options: ${JSON.stringify(options)}`);
    });
    ...
    expect(logs).toContain("Generating a new mnemonic with options: {\"strength\":128}");
    expect(logs.some(l => l.startsWith("Mnemonic imported successfully with ID:"))).toBe(true);
  });
});
